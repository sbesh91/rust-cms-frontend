...
import { article, listing } from "./templates";
...
class EditorPage extends LitElement {
  ...
  updated() {
    ...
    this.subscription = watch.pipe(debounceTime(1000)).subscribe((data) => {
  ...
  render() {
    ...
    return html`
      <header>
        ...
        <button class="small delete" @click=${(e) => this.delete()}>
          Delete
        </button>
        Section
      </header>
      <form @submit=${(e) => this.submit(e)}>
        <select
          ...
          name="section_type"
          @change=${(e) => this.setSectionType(e.target.value)}
        >
